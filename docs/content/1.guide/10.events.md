...
```ts
import type { ServerEventMap } from "sevo";
```
...
```ts
import { Server, ServerCloseEvent, ServerErrorEvent, ServerServeEvent } from "sevo";
...
In `sevo`, lifecycle events are dispatched internally by `Server`, but the
...
- `ServerErrorEvent` is part of `sevo` lifecycle semantics, not `evt`'s optional
