...
> Low-level helpers from `servok` for custom integration, advanced middleware composition, and framework authoring.
...
- integrating `servok` into another host runtime
- building higher-level abstractions on top of `servok`
...
```ts
import { raceRequestAbort } from "servok";
...
```ts
import { createWaitUntil } from "servok";
