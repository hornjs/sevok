...
adapter with dynamic imports like `import("servok/bun")`,
`import("servok/deno")`, or `import("servok/node")`.
...
`BunServerOptions` excludes `fetch`, `routes`, and `unix`, because `servok`
