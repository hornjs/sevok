...
```ts
import type { ServerEventMap } from "servok";
```
...
```ts
import { Server, ServerCloseEvent, ServerErrorEvent, ServerServeEvent } from "servok";
...
In `servok`, lifecycle events are dispatched internally by `Server`, but the
...
- `ServerErrorEvent` is part of `servok` lifecycle semantics, not `evt`'s optional
